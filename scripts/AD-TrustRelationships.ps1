<#
.SYNOPSIS
    AD Trust Relationships - Enhanced Dashboard
.DESCRIPTION
    Standalone dashboard that enumerates every Active Directory trust and assesses its
    security posture. Emits a single self-contained interactive HTML report (dark/light)
    with an insights dashboard, KPI tiles, and a trust explorer (list + detail panel).

    For each trust it collects:
      - Partner domain, direction (inbound / outbound / bidirectional), trust type
        (external / forest / realm / parent-child / tree-root / unknown), transitivity.
      - SECURITY: SID filtering (quarantine) state, selective authentication, TGT
        delegation, forest-transitive flag, AES/SHA support where available.
      - Lifecycle: created / modified dates, age, and a connectivity probe.

    Connectivity is tested per trust, but DEGRADES GRACEFULLY: in locked-down
    environments (no outbound auth, unreachable partner, insufficient rights) the result
    is reported as "Unavailable / Not tested" instead of a misleading failure.

    Security findings are derived (not hardcoded):
      - SID filtering DISABLED on an external/forest trust  -> high risk (SID history)
      - Selective authentication OFF on external trust       -> elevated exposure
      - Outbound/bidirectional external trust                 -> review
      - Stale trust (not modified in a long time)             -> review

    No hardcoded paths or names - everything is discovered live from the current domain.
.PARAMETER OutputPath
    Folder to save the HTML report. Defaults to the current directory.
.PARAMETER TestConnectivity
    Probe each trust partner (default: $true). Use -TestConnectivity:$false to skip.
...
<title>Trust Relationships — $DomainDNS</title>
...
<div class="wrap">
  <div class="section-label"><i class="bi bi-diagram-3"></i> Trust Topology</div>
  <div class="graph-card">
    <div class="graph-head"><i class="bi bi-share"></i> Interactive Trust Map
      <div class="graph-tools">
        <button onclick="graphReset()"><i class="bi bi-arrow-counterclockwise"></i> Reset</button>
        <button onclick="graphFit()"><i class="bi bi-aspect-ratio"></i> Fit</button>
      </div>
    </div>
    <div class="graph-legend">
      ...
      <div class="lg"><svg class="shp" viewBox="0 0 13 13"><rect x="1" y="3" width="11" height="7" rx="1.5" fill="var(--accent)"/></svg> This domain</div>
      ...
      <div class="lg"><span class="ln oneway"></span> One-way</div>
      <div class="lg"><span class="ln risk"></span> High risk</div>
    </div>
    <svg id="graphSvg"></svg>
  </div>

  <div class="section-label"><i class="bi bi-bar-chart-line"></i> Insights Dashboard</div>
  <div id="connNote"></div>
  <div class="dash-grid" id="dashboard"></div>

  <div class="section-label"><i class="bi bi-grid-1x2"></i> Overview</div>
  <div class="kpi-grid" id="kpis"></div>
  <div id="filterBanner" style="display:none"></div>

  <div class="section-label"><i class="bi bi-shuffle"></i> Trust Explorer</div>
  <div class="layout">
    <div class="panel-card">
      <div class="panel-head"><i class="bi bi-list-ul"></i> Trusts</div>
      <div class="list-search"><input id="trustSearch" type="text" placeholder="Filter trusts by name…" oninput="filterList()"></div>
      <div class="trust-list" id="trustList"></div>
    </div>
    <div class="panel-card">
      <div class="panel-head"><i class="bi bi-info-circle"></i> Details</div>
      <div class="detail" id="detail">
        <div class="detail-empty"><i class="bi bi-hand-index-thumb"></i>Select a trust to view its direction, type, and security attributes.</div>
      </div>
    </div>
  </div>
</div>

<div class="modal-overlay" id="trustModal" onclick="if(event.target===this)closeTrustModal()"><div class="modal-box"><button class="modal-close" onclick="closeTrustModal()" aria-label="Close">&times;</button><div class="detail" id="modalBody"></div></div></div>
...
const FILTERS = {
  all:      {test:t=>true,                         label:'All Trusts', icon:'bi-shuffle'},
  external: {test:t=>!t.withinForest,              label:'External Trusts', icon:'bi-box-arrow-up-right'},
  nosid:    {test:t=>!t.withinForest&&!t.sidFiltering, label:'SID Filtering Disabled', icon:'bi-shield-slash'},
  high:     {test:t=>t.severity==='high',          label:'High-Risk Trusts', icon:'bi-exclamation-octagon'},
  nosel:    {test:t=>!t.withinForest&&!t.selectiveAuth, label:'Selective Auth Off', icon:'bi-door-open'},
  bidir:    {test:t=>t.direction==='Bidirectional',label:'Bidirectional Trusts', icon:'bi-arrow-left-right'},
  unreach:  {test:t=>t.connectivity==='Unreachable', label:'Unreachable Trusts', icon:'bi-plug'}
};

function renderConnNote(){
  const el=document.getElementById('connNote');
  if(!D.connectivityRun){
    el.innerHTML = '<div class="banner-note"><i class="bi bi-info-circle"></i> Connectivity testing was skipped for this run — partner reachability is shown as "Not tested".</div>';
  } else if((D.unreachable||0)>0){
    el.innerHTML = '<div class="banner-note"><i class="bi bi-info-circle"></i> Some partners could not be reached. In locked-down environments this may reflect blocked outbound auth rather than a broken trust.</div>';
  } else { el.innerHTML=''; }
}
...
function renderKpis(){
  ...
  function kpi(v,l,ico,cls,fkey){
    ...
    const hint = fkey ? '<div class="hint">Filter View <i class="bi bi-arrow-right"></i></div>' : '';
  ...
  k.innerHTML =
    kpi(D.highRisk||0,'High Risk','bi-exclamation-octagon',((D.highRisk||0)>0?'k-red':'k-green'),'high')
    + kpi((D.posture&&D.posture.selOff)||0,'Selective Auth Off','bi-door-open',(((D.posture&&D.posture.selOff)||0)>0?'k-amber':'k-green'),'nosel')
    + kpi(D.noSidFilter||0,'SID Filtering Off','bi-shield-slash',((D.noSidFilter||0)>0?'k-red':'k-green'),'nosid')
...
function renderDashboard(){
  ...
  html += '<div class="chart-card col-6"><h3><i class="bi bi-pie-chart"></i> Risk Posture</h3>'
    + donutSvg([{v:ok,c:'var(--green)'},{v:lo,c:'var(--blue)'},{v:me,c:'var(--amber)'},{v:hi,c:'var(--red)'}], D.totalTrusts||0, 'trusts', ['OK','Low','Medium','High'])
    + '</div>';

  // Card 2: trusts by kind (bars)
  const kd = Array.isArray(D.kindDist)?D.kindDist:[];
  let c2='<div class="chart-card col-6"><h3><i class="bi bi-diagram-2"></i> Trusts by Type</h3>';
  ...
  } else { c2+='<div class="muted-note">No trusts.</div>'; }
  c2+='</div>'; html+=c2;
...
function fctx(fkey,t){
  switch(fkey){
    case 'high': return '<span style="color:var(--red)">high</span>';
    case 'nosel': return '<span style="color:var(--amber)">selective auth off</span>';
    case 'nosid': return '<span style="color:var(--red)">SID filter off</span>';
...
function applyFilter(){
  ...
  if(!matches.length){
    banner.innerHTML='<div class="filter-banner" style="border-color:var(--border);box-shadow:var(--shadow)"><div class="filter-banner-head" style="color:var(--muted)"><i class="bi '+f.icon+'"></i> No trusts match: '+f.label+'<button class="btn" onclick="toggleFilter(\''+activeFilter+'\')" style="margin-left:auto;padding:4px 10px;font-size:12px">Clear <i class="bi bi-x-lg"></i></button></div></div>';
    document.querySelectorAll('.trust-item.dimmed').forEach(n=>n.classList.remove('dimmed'));
    return;
  }
  banner.innerHTML='<div class="filter-banner"><div class="filter-banner-head"><i class="bi '+f.icon+'"></i> Viewing '+matches.length+' trust(s): '+f.label
    + '<button class="btn" onclick="toggleFilter(\''+activeFilter+'\')" style="margin-left:auto;padding:4px 10px;font-size:12px">Clear Filter <i class="bi bi-x-lg"></i></button></div>'
...
function renderList(){
  ...
  if(!TR.length){ el.innerHTML='<div class="muted-note">No trusts found in this domain.</div>'; return; }
  ...
  el.innerHTML = sorted.map(t=>{
    ...
    return '<div class="trust-item" data-name="'+encodeURIComponent(t.name)+'" onclick="onItemClick(this,\''+encodeURIComponent(t.name)+'\')">'
      ...
      + (t.withinForest?'':(t.sidFiltering?' · <span style="color:var(--green)">SID filter on</span>':' · <span style="color:var(--red)">SID filter off</span>'))+'</div></div>'
...
function boolKV(k,val,goodWhenTrue,goodTxt,badTxt){
  ...
  const txt = val ? (goodTxt||'Yes') : (badTxt||'No');
...
function connKV(c){
  ...
  return kv('Connectivity','<i class="bi '+icon+'"></i> '+c, cls);
}
function buildDetailHtml(name){
  ...
  const grid = '<div class="kv-grid">'
    ...
    + kv('Direction', escapeHtml(t.direction))
    + kv('Trust Type', escapeHtml(t.trustType))
    + kv('Classification', escapeHtml(t.kind))
    + boolKV('Transitive', t.transitive, true, 'Transitive', 'Non-transitive')
    + connKV(t.connectivity)
    + '</div>';
  let secGrid;
  if(t.withinForest){
    secGrid = '<div class="muted-note">Intra-forest trust — SID filtering and selective authentication are managed automatically within the forest.</div>';
  } else {
    secGrid = '<div class="kv-grid">'
      + boolKV('SID Filtering', t.sidFiltering, true, 'Enabled (quarantined)', 'DISABLED')
      + boolKV('Selective Auth', t.selectiveAuth, true, 'Enabled', 'Open (domain-wide)')
      + boolKV('Forest Transitive', t.forestTransitive, false, 'Yes', 'No')
      + boolKV('RC4 Permitted', t.usesRC4, false, 'Yes (weak)', 'No')
      + '</div>';
  }
  const lifeGrid = '<div class="kv-grid">'
    + kv('Created', escapeHtml(t.created||'—'),'mut')
    + kv('Modified', escapeHtml(t.modified||'—'),'mut')
    + '</div>';
  return (
    ...
    + '<div class="dsec"><div class="dsec-t"><i class="bi bi-diagram-2"></i> Trust Properties</div>'+grid+'</div>'
    + '<div class="dsec"><div class="dsec-t"><i class="bi bi-shield-lock"></i> Security Attributes</div>'+secGrid+'</div>'
    + '<div class="dsec"><div class="dsec-t"><i class="bi bi-clock-history"></i> Lifecycle</div>'+lifeGrid+'</div>'
    + '</div>');
}
...
function toggleTheme(){
  ...
  btn.innerHTML = isDark ? '<i class="bi bi-moon-stars"></i> Theme' : '<i class="bi bi-sun"></i> Theme';
}
...
function initGraphLayout(){
  ...
  if(ourDoms.length){
    ...
    FORESTS.push({ key:ourKey, label:ourKey+' (this forest)', isExternal:false, domains:ourDoms, x:leftX, y:fy, w:fw, h:leftH });
  }
...
function renderGraph(){
  ...
  if(!GNODES.length){ gSvg.innerHTML='<text x="50%" y="50%" text-anchor="middle" fill="var(--muted)" font-size="13">No trusts to graph.</text>'; return; }
  ...
  GNODES.forEach(n=>{
    ...
    const roleTxt = n.type==='self'?'this domain':(n.type==='intraforest'?'domain':'domain');
